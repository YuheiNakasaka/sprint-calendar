 ...
 *   - 2025/03/18(火)がリリース日
 */
import { addDays, getDayOfWeek, isSameDay } from './date-utils';
 ...
 */
export function getNearestDayOfWeek(
  dayOfWeek: number,
...
export function getAllPeriodsForDate(date: Date, sprintPeriods: SprintPeriod[]): PeriodInfo[] {
  ...
  // リリース日の昇順でソート
  const sortedPeriods = [...sprintPeriods].sort(
  ...
  for (const sprint of sortedPeriods) {
    ...
    if (type !== 'none') {
      ...
      if (existingIndex !== -1) {
        ...
        if (
          type === 'release' ||
          (type === 'qa' && results[existingIndex].type === 'development')
        ) {
 ...
 * @returns スプリント期間情報
 */
export function calculateSprintPeriods(
  startDate: Date,
  devDays: number,
  qaDays: number
): SprintPeriod {
 ...
 */
export function getNextNSprints(
  startDate: Date,
  devDays: number,
  qaDays: number,
  count: number
 ...
 * @returns スプリント期間情報の配列
 */
export function getPreviousNSprints(
  startDate: Date,
  devDays: number,
  qaDays: number,
  count: number
